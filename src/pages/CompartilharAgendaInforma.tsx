...
export default function CompartilharAgendaInforma() {
  ...
  const prepareShare = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("submissions")
      .select(
        "id, status, event_title, date, start_time, location, address_street, address_number, address_neighborhood, address_city, address_state, atrativo_name, submission_atrativos(name, display_order), estabelecimento:estabelecimentos(endereco, numero, bairro)",
      )
