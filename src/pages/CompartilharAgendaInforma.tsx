...
export default function CompartilharAgendaInforma() {
  ...
  const prepareShare = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("submissions")
      .select(
        "id, status, event_title, date, start_time, location, estabelecimento_name, address_street, address_number, address_neighborhood, atrativo_name, submission_atrativos(name, display_order)",
      )
